const state={token:localStorage.getItem("goldspot_token"),user:null,symbol:null,prices:[],candles:[],analysis:null,lastSentSignal:null,markets:[],scanResults:[],scanIndex:0,scanReq:1000,scanBusy:false,pendingScan:null,scanTimer:null};
...
const tickerMessages=[["LIVE","Real MT5 broker connection"],["MT5","Enter only your own MT5 login details"],["MARKET","Prices come from your connected MT5 broker"],["XAU","Gold symbol is selected from the broker’s real symbols"],["SECURE","MetaApi token stays on the server"],["STATUS","No fake balance or fake broker connection"]];
...
async function setBot(on){if(on&&!state.user?.mt5?.connected){toast("Connect your MT5 account first. The server handles MetaApi automatically.");return}await api(on?"/api/bot/start":"/api/bot/stop",{method:"POST",body:"{}"});state.user.settings.autoTrade=on;applyUser()}
...
function aggregateCandles(candles,seconds){
  ...
  for(const x of candles){
    const t=Math.floor(Number(x.time)/step)*step;
    const p={open:Number(x.open),high:Number(x.high),low:Number(x.low),close:Number(x.close),volume:Number(x.volume||0),time:t};
    if(!Number.isFinite(p.close))continue;
    const old=buckets.get(t);
    if(!old)buckets.set(t,p);
    else{old.high=Math.max(old.high,p.high);old.low=Math.min(old.low,p.low);old.close=p.close;old.volume+=p.volume}
  }
  return [...buckets.values()].sort((a,b)=>a.time-b.time).slice(-200);
}
async function loadMetaMarkets(){
  try{
...
function startMarketScan(){if(state.scanBusy||!state.markets.length||state.ws?.readyState!==1)return;state.scanBusy=true;state.scanResults=[];state.scanIndex=0;state.pendingScan=null;$("market-scan-status").textContent="Scanning 0 / "+state.markets.length;renderMarketResults();scanNextMarket()}
function scanNextMarket(){if(state.pendingScan)return;if(state.scanIndex>=state.markets.length){state.scanBusy=false;$("market-scan-status").textContent="Complete • "+state.scanResults.length+" analyzed";renderMarketResults();return}const m=state.markets[state.scanIndex++],req=state.scanReq++;state.pendingScan={req,market:m};$("market-scan-status").textContent="Scanning "+state.scanIndex+" / "+state.markets.length;if(state.ws?.readyState!==1){state.scanBusy=false;return}state.ws.send(JSON.stringify({ticks_history:m.symbol,end:"latest",style:"candles",granularity:Number($("timeframe").value),count:100,subscribe:0,req_id:req}))}
function handleScanHistory(d){if(!state.pendingScan||d.req_id!==state.pendingScan.req)return;const candles=d.candles||[];const a=analyzeMarketCandles(candles);if(a)state.scanResults.push({...state.pendingScan.market,...a});else state.scanResults.push({...state.pendingScan.market,last:null,trend:"NO DATA",r:null,side:"WAIT",strength:"WAIT"});state.pendingScan=null;renderMarketResults();setTimeout(scanNextMarket,80)}
function renderMarketResults(){const total=state.markets.length,buy=state.scanResults.filter(x=>x.side==="BUY").length,sell=state.scanResults.filter(x=>x.side==="SELL").length;$("market-summary").innerHTML='<div class="market-stat"><span>ACTIVE MARKETS</span><b>'+total+'</b></div><div class="market-stat"><span>BUY SIGNALS</span><b>'+buy+'</b></div><div class="market-stat"><span>SELL SIGNALS</span><b>'+sell+'</b></div><div class="market-stat"><span>ANALYZED</span><b>'+state.scanResults.length+'</b></div>';const sorted=[...state.scanResults].sort((a,b)=>(a.side==="WAIT"?1:0)-(b.side==="WAIT"?1:0)||a.name.localeCompare(b.name));$("market-list").innerHTML=sorted.length?'<div class="market-row market-head"><span>MARKET</span><span>TYPE</span><span>PRICE</span><span>TREND</span><span>RSI</span><span>SIGNAL</span></div>'+sorted.map(x=>'<div class="market-row"><b>'+escapeHtml(x.name)+'</b><span>'+escapeHtml(x.market||"")+'</span><span>'+Number.isFinite(Number(x.last))?Number(x.last).toFixed(5):"—"+'</span><span>'+x.trend+'</span><span>'+Number(x.r).toFixed(1)+'</span><span class="'+x.side.toLowerCase()+'">'+x.side+'</span></div>').join(""):'<div class="empty">Waiting for market scan...</div>'}
function handleScanError(d){if(state.pendingScan){state.scanResults.push({...state.pendingScan.market,last:null,trend:"NO DATA",r:null,side:"WAIT",strength:"WAIT"});state.pendingScan=null;renderMarketResults()}if(state.scanBusy)setTimeout(scanNextMarket,120);$("market-scan-status").textContent="MetaApi data retrying...";if(!state.markets.length)tryKnownGoldSymbols()}
...
$("mt5-form").addEventListener("submit",async e=>{e.preventDefault();try{const r=await api("/api/mt5/connect",{method:"POST",body:JSON.stringify({broker:$("m-broker").value,server:$("m-server").value,login:$("m-login").value,password:$("m-password").value})});state.user.mt5=r.mt5;applyUser();$("mt5-message").textContent=r.message;let n=0;const poll=setInterval(async()=>{n++;try{const x=await api("/api/mt5/status");state.user.mt5=x.mt5;applyUser();if(x.mt5.setupStatus==="READY"||x.mt5.setupStatus==="ERROR"){clearInterval(poll);$("mt5-message").textContent=x.mt5.setupStatus==="READY"?"MetaApi Cloud connected — no local MT5 terminal is required.":"MetaApi: "+(x.mt5.error||"connection failed");if(x.mt5.setupStatus==="READY")loadMetaMarkets()}}catch{}if(n>=40)clearInterval(poll)},3000)}catch(err){$("mt5-message").textContent=err.message}});
